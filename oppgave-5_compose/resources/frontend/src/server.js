...
const backendServer = backend.listen(backend.get('port'), function () {
    console.log('Server listening on port ' + backend.get('port') + ' in ' + backend.get('env') + ' mode');
});
...
const frontendServer = frontend.listen(frontend.get('port'), function () {
    console.log('Client listening on port ' + frontend.get('port') + ' in ' + frontend.get('env') + ' mode');
});